...
export class ChatLayoutComponent implements AfterViewChecked {
  ...
  ngOnInit() {
    ...
    this.getAllUsers();
    this.getMe();
    this.socket.listen<any>('receivePrivateMessage').subscribe((msg) => {
    ...
    this.socket.listen<{ from: string }>('typing').subscribe(({ from }) => {
    ...
    this.socket.listen<{ from: string }>('stopTyping').subscribe(({ from }) => {
      if (from === this.user2Data._id) {
  ...
  getPrivateMessages() {
    ...
    this.chatService
      .getPrivateMessages(this.user1Data._id, this.user2Data._id)
      .subscribe({
        next: (res: any) => {
  ...
  getUserImage(senderId: string): string {
    ...
    return user.image !== null
      ? user.image
      : 'public/images/chat-background.jpg';
  }
  ...
  getUser(id: string) {
    ...
    this.user2Data = user!;
    this.chatService
      .createOrGetRoom([this.user1Data._id, id])
      .subscribe((res: any) => {
        console.log(res);
  ...
  loadRoomMessages() {
    this.chatService
      .getRoomMessages(this.currentRoomId)
      .subscribe((msgs) => (this.privateMessages = msgs));
  }

  getRecentPrivateMessage() {
    this.chatService.getRecentPrivate().subscribe({
      next: (res: any) => {
        this.recentPrivateMessage = res;
        console.log(res);
      },
      error: (err: any) => {
        console.log(err);
  ...
  getRecentGroupeMessage() {
    this.chatService.getRecentGroup().subscribe({
      next: (res: any) => {
        this.recentGroupMessages = res;
        console.log(res);
      },
      error: (err: any) => {
        console.log(err);
  ...
  sendMessage() {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        this.getRecentPrivateMessage();
        this.getRecentGroupeMessage();
        this.socket.emit('stopTyping', {
          to: this.user2Data._id,
          userId: this.user1Data._id,
        });
