...
export class ChatService {
  ...
  searchUsers(q: string): Observable<User[]> {
    return this.http
      .get<User[]>(`${this.base}/users-filter`, {
  ...
  blockUser(targetUserId: string): Observable<any> {
    return this.http
      .post(`${this.base}/block`, { targetUserId })
      .pipe(catchError(this.handleErr));
  }

  unblockUser(targetUserId: string): Observable<any> {
    return this.http
      .post(`${this.base}/unblock`, { targetUserId })
      .pipe(catchError(this.handleErr));
  }
  ...
  sendMessage(
    ...
    return this.http
      .post<Message>(`${this.base}/send`, body)
      .pipe(catchError(this.handleErr));
  }
  ...
  sendFile(
    ...
    return this.http
      .post<Message>(`${this.base}/send-file`, fd)
      .pipe(catchError(this.handleErr));
  }

  getRoomMessages(roomId: string): Observable<Message[]> {
    return this.http
      .get<Message[]>(`${this.base}/room/${roomId}/messages`)
      .pipe(catchError(this.handleErr));
  }

  getPrivateMessages(user1: string, user2: string): Observable<Message[]> {
    return this.http
      .get<Message[]>(`${this.base}/private/${user1}/${user2}`)
      .pipe(catchError(this.handleErr));
  }

  markRead(messageIds: string[]): Observable<any> {
    return this.http
      .post(`${this.base}/mark-read`, { messageIds })
      .pipe(catchError(this.handleErr));
  }

  getRecentPrivate(): Observable<Message[]> {
    return this.http
      .get<Message[]>(`${this.base}/private/recent`)
      .pipe(catchError(this.handleErr));
  }

  getRecentGroup(): Observable<Message[]> {
    return this.http
      .get<Message[]>(`${this.base}/group/recent`)
      .pipe(catchError(this.handleErr));
  }
